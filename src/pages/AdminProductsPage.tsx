...
const AdminProductsPage: React.FC<AdminProductsPageProps> = () => {
  ...
  const handleDeleteProduct = async (productId: string) => {
    ...
    if (success) {
      ...
      setFeaturedProducts(
        featuredProducts.filter((fp) => fp.product_id !== productId),
      );
  ...
  const handleAddFeaturedProduct = async (e: React.FormEvent) => {
    ...
    const displayOrder =
      Math.max(...featuredProducts.map((fp) => fp.display_order), 0) + 1;
  ...
  const handleRemoveFeatured = async (featuredId: string) => {
    ...
    const success =
      await featuredProductService.removeFeaturedProduct(featuredId);
    if (success) {
      setFeaturedProducts(
        featuredProducts.filter((fp) => fp.id !== featuredId),
      );
  ...
  const handleToggleFeatured = async (
    ...
    const updated = await featuredProductService.toggleFeaturedProduct(
      featuredId,
      !isActive,
    );
    if (updated) {
      setFeaturedProducts(
        featuredProducts.map((fp) => (fp.id === featuredId ? updated : fp)),
      );
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">
            Create products and manage featured items
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            Featured Products (
            {featuredProducts.filter((fp) => fp.is_active).length})
          </h2>
          <button
        ...
        {featuredProducts.length > 0 ? (
          <div className="space-y-3">
            {featuredProducts.map((fp) => (
              ...
              >
                ...
                <img
                  src={
                    fp.product?.image_url || "https://via.placeholder.com/50"
                  }
                  alt={fp.product?.name}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-400">
                    ₱{fp.product?.unit_price.toLocaleString()} • Category:{" "}
                    {fp.product?.category}
                  </p>
                </div>
          ...
          <p className="text-center text-gray-400 py-8">
            No featured products yet. Add some to get started!
          </p>
        )}
      </motion.div>
                ...
                <div className="flex-1">
                  {editingId === product.id ? (
                    ...
                    <>
                      ...
                      <p className="text-sm text-gray-400">
                        SKU: {product.sku} • ₱
                        {product.unit_price.toLocaleString()} • Stock:{" "}
